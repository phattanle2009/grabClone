import 'package:flutter/material.dart';

class AppColors {
  static const green = Color(0xFF32C072);
  static const mediumGreen = Color(0xFF02B14F);
  static const darkGeen = Color(0xFF0005238);
  static const lightGreen = Color(0xFFCCEFDC);
  static const mediumGrey = Color(0xFFF2F2F3);
  static const lightGray = Color(0xFF9A9A9A);
  static const lighterGrey = Color(0xFFF0F2EC);
  static const lightBlue = Color(0xFFEEF9F9);
  static const lightPink = Color(0xFFFFF4EA);
  static const red = Color(0xFFF55439);
  static const lightPurple = Color(0xFF6CA5F3);
  static const lighterPurple = Color(0xFF83B5F5);
  static const lighterBlue = Color(0xFF42C0E7);
  static const mediumPink = Color(0xFFFCDDF1);
  static const iconBackgroundGrey = Color(0xFFE4E8F0);
  static const orange = Color(0xFFF38422);
  static const background = Color(0xFFF6F6F7);
}
